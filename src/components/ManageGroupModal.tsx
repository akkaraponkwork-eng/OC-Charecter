...
export default function ManageGroupModal({ chat, onClose, onUpdated }: Props) {
  const { data: session } = useSession();
  const uid = (session?.user as any)?.uid;
  const [name, setName] = useState(chat.title || '');
  const [loading, setLoading] = useState(false);
  const [users, setUsers] = useState<any[]>([]);
  const [selectedIds, setSelectedIds] = useState<string[]>(chat.memberIds || []);
  ...
  const toggleUser = (userId: string) => {
    if (userId === chat.ownerId) return; // Cannot remove owner
    setSelectedIds(prev => 
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            ...
            <div>
              <div style={{ fontWeight: 700, fontSize: '1rem', color: 'var(--text-main)' }}>จัดการกลุ่มแชท</div>
              <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Manage group members & settings</div>
            </div>
          </div>
          <button
            onClick={onClose}
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                        ...
                        <div>
                          <div style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-main)', display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                            ...
                            {isOwner && <Shield size={12} color="var(--accent)" title="Owner" />}
                          </div>
